...
def test_diagnostics_channel_channel(koss: KossJS):
    result = koss.eval("typeof require('diagnostics_channel').channel('test')")
    assert result == 'object'


def test_diagnostics_channel_has_subscribers_false(koss: KossJS):
    result = koss.eval("var dc = require('diagnostics_channel'); dc.hasSubscribers('test')")
...
def test_diagnostics_channel_channel_name(koss: KossJS):
    result = koss.eval("require('diagnostics_channel').channel('test4').name")
    assert result == 'test4'
