...
class TestBufferAPI:
    def test_require_buffer(self, koss: KossJS):
        result = koss.eval("typeof require('koss:node/buffer').Buffer")
    ...
    def test_btoa(self, koss: KossJS):
        result = koss.eval(
            "const {btoa} = require('koss:node/buffer'); "
            "btoa('hello')"
        )
        assert result == "aGVsbG8="
    ...
    def test_atob(self, koss: KossJS):
        result = koss.eval(
            "const {atob} = require('koss:node/buffer'); "
            "atob('aGVsbG8=')"
        )
        assert result == "hello"
    ...
    def test_buffer_constants(self, koss: KossJS):
        result = koss.eval(
            "const {constants} = require('koss:node/buffer'); "
            "typeof constants"
        )
    ...
    def test_buffer_k_max_length(self, koss: KossJS):
        result = koss.eval(
            "const {kMaxLength} = require('koss:node/buffer'); "
            "typeof kMaxLength"
        )
